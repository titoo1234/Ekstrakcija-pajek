...
class Baza():
    ...
    def preveri_in_dodaj_domeno(self,link2,vmesnik):
        if self.poglej_domeno(link2)[0] == 0: #Domena še ne obstaja
            ...
            try:
                ...
                self.dodaj_domeno(robotsfile.domena, robotsfile.vsebina, robotsfile.sitemap,5)
    ...
    def posodobi_page(self, site_id, page_type_code, url, html_content, http_status_code, accessed_time):
        ...
        if page_type_code == "DUPLICATE":
            cur.execute(poizvedba, (site_id, page_type_code, url, str(self.najdi_duplikat(html_content)), http_status_code, accessed_time, url))
        else:
    ...
    def dodaj_page_v_bazo(self, site_id, page_type_code, url, html_content, http_status_code, accessed_time):
        ...
        print('Dodan novi page v bazo:',url)
        id = cur.fetchone()[0]
        cur.close()
        return id
    ...
    def je_duplikat(self,vsebina):
        cur = self.conn.cursor()
        poizvedba = "SELECT COUNT(*) from crawldb.page WHERE html_content = %s and  html_content is not null"
    ...
    def najdi_duplikat(self, vsebina):
        cur = self.conn.cursor()
        poizvedba = "SELECT id from crawldb.page WHERE html_content = %s"
        cur.execute(poizvedba, (vsebina,))
        id = cur.fetchone()[0]
        cur.close()
        return id
